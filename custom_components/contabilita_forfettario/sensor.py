...
class ContabilitaBaseSensor(SensorEntity):
    ...
    def __init__(self, unique_id: str, name: str, unit: str | None = None):
        ...
        self._attr_name = f"Contabilità Forfettario {name}"
...
class RicaviAnnuiSensor(ContabilitaBaseSensor):
    ...
    def __init__(self):
        super().__init__("ricavi_annui_2", "Ricavi Annui 2")
        self._attr_native_unit_of_measurement = "€"
        self._attr_state_class = SensorStateClass.TOTAL

    def get_dependencies(self) -> list[str]:
        return ["number.contabilita_forfettario_anno_inizio_attivita",
                "number.contabilita_forfettario_coefficiente_redditivita",
                "number.contabilita_forfettario_contributi_inps_anno_precedente",
                "number.contabilita_forfettario_ricavi_annui",
                "number.contabilita_forfettario_imposta_netta_anno_precedente"]

    @property
    def native_value(self):
        return self.get_state_value("number.contabilita_forfettario_ricavi_annui")
...
class RedditoForfettarioSensor(ContabilitaBaseSensor):
    ...
    @property
    def native_value(self):
        ricavi = self.get_state_value("number.contabilita_forfettario_ricavi_annui")
        coeff = self.get_state_value("number.contabilita_forfettario_coefficiente_redditivita", 0.78)
...
class ContributiINPSSensor(ContabilitaBaseSensor):
    ...
    def get_dependencies(self) -> list[str]:
        return ["sensor.contabilita_forfettario_reddito_forfettario"]

    @property
    def native_value(self):
        reddito = self.get_state_value("sensor.contabilita_forfettario_reddito_forfettario")
...
class AliquotaImpostaForfettarioSensor(ContabilitaBaseSensor):
    ...
    def native_value(self):
        ...
        anno_start = int(self.get_state_value("number.contabilita_forfettario_anno_inizio_attivita", anno_corrente))
...
class BaseImponibileImpostaSensor(ContabilitaBaseSensor):
    ...
    def get_dependencies(self) -> list[str]:
        return [
            "sensor.contabilita_forfettario_reddito_forfettario",
            "sensor.contabilita_forfettario_contributi_inps",
        ]

    @property
    def native_value(self):
        reddito = self.get_state_value("sensor.contabilita_forfettario_reddito_forfettario")
        inps = self.get_state_value("sensor.contabilita_forfettario_contributi_inps")
...
class ImpostaSostitutivaSensor(ContabilitaBaseSensor):
    ...
    def get_dependencies(self) -> list[str]:
        return [
            "sensor.contabilita_forfettario_base_imponibile_imposta",
            "sensor.contabilita_forfettario_aliquota_imposta_forfettario",
        ]

    @property
    def native_value(self):
        base = self.get_state_value("sensor.contabilita_forfettario_base_imponibile_imposta")
        aliquota = self.get_state_value("sensor.contabilita_forfettario_aliquota_imposta_forfettario")
...
class CaricoFiscaleRealeSensor(ContabilitaBaseSensor):
    ...
    def get_dependencies(self) -> list[str]:
        return [
            "sensor.contabilita_forfettario_contributi_inps",
            "sensor.contabilita_forfettario_imposta_sostitutiva",
        ]

    @property
    def native_value(self):
        inps = self.get_state_value("sensor.contabilita_forfettario_contributi_inps")
        imposta = self.get_state_value("sensor.contabilita_forfettario_imposta_sostitutiva")
...
class UtileNettoStimatoSensor(ContabilitaBaseSensor):
    ...
    def get_dependencies(self) -> list[str]:
        return [
            "number.contabilita_forfettario_ricavi_annui",
            "sensor.contabilita_forfettario_carico_fiscale_reale",
        ]

    @property
    def native_value(self):
        ricavi = self.get_state_value("number.contabilita_forfettario_ricavi_annui")
        carico = self.get_state_value("sensor.contabilita_forfettario_carico_fiscale_reale")
...
class RicaviAnnoPrecedenteSensor(ContabilitaBaseSensor):
    ...
    def get_dependencies(self) -> list[str]:
        return ["sensor.contabilita_forfettario_ricavi_annui_2_last_year"]

    @property
    def native_value(self):
        # Questo dipende dal utility_meter configurato dall'utente
        return self.get_state_value("sensor.contabilita_forfettario_ricavi_annui_2_last_year")
...
class RedditoForfettarioAnnoPrecedenteSensor(ContabilitaBaseSensor):
    ...
    def get_dependencies(self) -> list[str]:
        return [
            "sensor.contabilita_forfettario_ricavi_anno_precedente",
            "number.contabilita_forfettario_coefficiente_redditivita",
        ]

    @property
    def native_value(self):
        ricavi = self.get_state_value("sensor.contabilita_forfettario_ricavi_anno_precedente")
        coeff = self.get_state_value("number.contabilita_forfettario_coefficiente_redditivita", 0.78)
        return round(ricavi * coeff, 2)
...
class BaseImponibileAnnoPrecedenteSensor(ContabilitaBaseSensor):
    ...
    def get_dependencies(self) -> list[str]:
        return [
            "sensor.contabilita_forfettario_reddito_forfettario_anno_precedente",
            "number.contabilita_forfettario_contributi_inps_anno_precedente",
        ]

    @property
    def native_value(self):
        reddito = self.get_state_value("sensor.contabilita_forfettario_reddito_forfettario_anno_precedente")
        inps = self.get_state_value("number.contabilita_forfettario_contributi_inps_anno_precedente")
        base = reddito - inps
...
class ImpostaNettaAnnoPrecedenteSensor(ContabilitaBaseSensor):
    ...
    def get_dependencies(self) -> list[str]:
        return [
            "sensor.contabilita_forfettario_base_imponibile_anno_precedente",
            "number.contabilita_forfettario_anno_inizio_attivita",
        ]

    @property
    def native_value(self):
        base = self.get_state_value("sensor.contabilita_forfettario_base_imponibile_anno_precedente")
        if base <= 0:
            return 0
...
class AccontoTotaleImpostaSensor(ContabilitaBaseSensor):
    ...
    def get_dependencies(self) -> list[str]:
        return ["sensor.contabilita_forfettario_imposta_netta_anno_precedente"]

    @property
    def native_value(self):
        imp = self.get_state_value("sensor.contabilita_forfettario_imposta_netta_anno_precedente")
...
class AccontoGiugnoSensor(ContabilitaBaseSensor):
    ...
    def get_dependencies(self) -> list[str]:
        return ["sensor.contabilita_forfettario_acconto_totale_imposta"]

    @property
    def native_value(self):
        totale = self.get_state_value("sensor.contabilita_forfettario_acconto_totale_imposta")
